 ...
 * @package Sentora
 * @version $Id$
 * @author Bobby Allen - ballen@sentora.org
 * @copyright (c) 2008-2011 Sentora Group - http://www.sentora.org/
...
class module_controller {
    ...
    static function ExecuteChangeClientPackage($clientid, $package, $group) {
        global $zdbh;
        
        if(empty($clientid) || empty($package) || empty($group)){
            return false;
        }
        
        $sql = $zdbh->prepare("UPDATE x_accounts SET ac_package_fk= :package, ac_group_fk= :group WHERE ac_id_pk = :clientid");
        $sql->bindParam(':package', $package);
        $sql->bindParam(':group', $group);
        $sql->bindParam(':clientid', $clientid);
        $sql->execute();

        $sql = $zdbh->prepare("UPDATE x_profiles SET ud_package_fk= :package, ud_group_fk= :group WHERE ud_user_fk=:accountid");
        $sql->bindParam(':group', $group);
        $sql->bindParam(':package', $package);
        $sql->bindParam(':accountid', $clientid);
        $sql->execute();
