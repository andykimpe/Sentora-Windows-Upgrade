...
class module_controller extends ctrl_module
{
    ...
    {
        global $zdbh;
        $sqlString = "SELECT * FROM zpanel_core.x_htpasswd_file 
    ...
    static function fileInPathCheck($file)
    {
        ...
        if( 0 !== strpos($realPath, realpath(self::getHostDir() . self::getCurrentUsername() . '/')))
        {
    ...
    static function buildHtaccessLink($message, $htpasswdFile) {
        $htaccessString = PHP_EOL . 'AuthName "' . $message . '"' . PHP_EOL .
    ...
    static function doCreateUser()
    {
        ...
        $encryptedPassword = self::crypt_apr1_md5($password);
