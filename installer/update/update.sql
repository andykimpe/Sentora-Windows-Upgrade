/* prepare for upgrade */
/* https://github.com/sentora/sentora-installers/blob/master/preconf/sentora-update/zpanel/sql/update-data.sql */
...
INSERT INTO `zpanel_core`.`x_permissions`(`pe_group_fk`,`pe_module_fk`) VALUES (3, (SELECT `mo_id_pk` FROM `zpanel_core`.`x_modules` WHERE `mo_folder_vc` = 'protected_directories' LIMIT 1));

/* https://github.com/sentora/sentora-installers/blob/master/preconf/sentora-update/zpanel/sql/update-structure.sql */
